...
def main():
    ...
    #better_draw_circles(rg.Point(-200,-10),5)
    #better_draw_circles(rg.Point(250,100), 20)
    even_better_draw_circles(rg.Point(260,100),20,6,'lavender',8)
    even_better_draw_circles(rg.Point(-100,-100),5,30,'cyan',2)
    window.update()
...
def better_draw_circles(point,rad):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        turtle.pen_down()
        turtle.draw_circle(rad * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point,rad,number_of_circles,color,thickness):
    turtle = rg.SimpleTurtle()
